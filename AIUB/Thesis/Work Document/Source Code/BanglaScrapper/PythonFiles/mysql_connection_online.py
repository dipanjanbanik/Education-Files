...
Last modified:      26-JUN-2018, 1:20 PM
...
def mysql_connection_func(sql_paragraph, sql_source_name, sql_date, sql_user_id, sql_domain_id, sql_source_link,
                          sql_sentence_count, sentence_dictionary, sql_title):
    try:
        ...
        insert_into_article = ('INSERT INTO article'
                               '(news_string, user_id, domain_id, sentence_count) '
                               'VALUES(%(news_string)s, %(u_id)s, %(domain_id)s, %(sentence_count)s)')
        ...
        insert_into_article_information = ('INSERT INTO article_information'
                               '(article_title, source, date, source_link, article_id) '
                               'VALUES(%(article_title)s, %(source)s, %(date)s, %(source_link)s, %(article_id)s)')

        article_information_values = {
            'article_title': sql_title,
            'source'       : sql_source_name,
            'date'         : sql_date,
            'source_link'  : sql_source_link,
            'article_id'   : rowl[0] }
        cur.execute(insert_into_article_information, article_information_values)
        connection_string.commit()
