...
class SDMScorer(Scorer):
    def unigram_matches(self, query_terms: List[str], doc: Entity) -> float:
        ...
        for term in query_terms:
            if doc.term_stats(term) != None:
                term_freq = doc.term_stats(term)['term_freq']
                total_term_freq = doc.term_stats(term)['ttf']
    ...
    def ordered_bigram_matches(
        ...
        bigram_list = [x for x in zip(query_terms[:-1],query_terms[1:])]
        terms = doc.terms()
        term_freq = sum([1 for i in bigram_list for c in range(len(terms)-1) if terms[c]==i[0]and terms[c+1]==i[1]])
        total_length = doc.field_stats()['sum_ttf']
        length = doc.length()

        for col in documents:
            t = col.terms()
            total_freq += sum([1 for i in bigram_list for c in range(len(t)-1) if t[c]==i[0]and t[c+1]==i[1]])
